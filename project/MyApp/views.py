        ...
        if len(user_result) == 1:#
            ...
            if user_password == user_password_:
                ...
                if len(user_r) == 0 :
                    ...
                    context["info"] = "Wrong identity！！！"
                    context["status"] = 3
                    return render(request, 'User/new_login.html', context=context)
            else:
                context["info"] = "Password uncorrect！！！"
                context["status"] = 1
                return render(request, 'User/new_login.html', context=context)
        else:
            context["info"] = "User dosen't exist！！！"
            context["status"] = 2
            return render(request, 'User/new_login.html', context=context)
...
def doctor_regist(request):
    if request.method == "GET":
        ...
        if user_result is None:
            context["info"] = "You are not docotr！！！"
            context["status"] = 1
            ...
            if len(user_result_u) == 0:
                ...
                context["info"] = "User already exist！！！"
                context["status"] = 2
                return render(request, 'User/doctor_regist.html', context=context)
def choose(request):
...
def search_by_patient_name(request):
        ...
        info_dic = {}
        info_dic["Consultation number"] = d.id
        global id2
        id2 = d.id
        info_dic["Name"] = d.patient_name
        info_dic["Gender"] = d.patient_sex
        info_dic["Age"] = d.patient_age
        info_dic["Telephone"] = d.patient_telep
        info_dic["Department"] = d.patient_dept.dept_name
        context = {
...
def search_by_doctor_name(request):
    ...
    info_dic = {}
    info_dic["Doctor_ID"] = d.doctor_id
    info_dic["Name"] = d.doctor_name
    info_dic["Gender"] = d.doctor_sex
    info_dic["Age"] = d.doctor_age
    info_dic["Telephone"] = d.doctor_telep
    info_dic["Position"] = d.doctor_position
    info_dic["Department"] = d.doctor_dept.dept_name
    id4=d.doctor_name
    context = {
        "result_keys": list(info_dic.keys()),
        "result_values": list(info_dic.values()),
...
def diagnosis_detail(request):
    ...
    if request.method=="GET":
        ...
        if c is None:
            return HttpResponse("This patient haven't been treated，no info！")
        else:
            info_dic = {}
            info_dic["id"] = d.id
...
def search_medicine(request):
    ...
    info_dic = {}
    info_dic["Drug_name"] = d.medicine_name
    medicine1=d.medicine_name
    info_dic["Dosis"] = d.medicine_num
...
def insert_deal_method(request):
    ...
    medicine_detail=""
    if medicine1.strip()!="":
        medicine_detail = "1、"+medicine1 + "  " + num1 + "box" + "\n"
        if medicine2.strip()!="":
            medicine_detail = medicine_detail+"2、"+medicine2 + "  " + num2 + "box" + "\n"
            if medicine3.strip() != "":
                medicine_detail = medicine_detail+"3、"+medicine3 + "  " + num3 + "box" + "\n"
            else:
                pass
        else:
            pass
...
def out_hospital(request):
    ...
    if request.method == "GET":
        ...
        Room.objects.filter(room_patient_dept=int(dept1) ,room_id=out_id).update(room_patient_name="empty",
            room_patient_id="empty")
